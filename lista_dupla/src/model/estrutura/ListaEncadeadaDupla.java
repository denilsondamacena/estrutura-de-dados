...
public class ListaEncadeadaDupla<T> {
	...
	public void insert(No<T> item, T elemento) throws IllegalArgumentException {
		...
		//correção: verifica se existe um próximo antes de usar
		if(proximo != null)
	...
	public void remove(int index) {
		...
		//correção: verifica se a lista está vazia
		if (this.inicio == null)
			throw new IllegalArgumentException("não existe item na lista.");
		if (index == 0) {
			...
			if ( this.inicio.getProximo() == null) {
				...
				//correção: novo início não possui anterior
		...
		//correção: variável próxima precisava ser declarada
		No<T> proximo = item.getProximo();
		anterior.setProximo(proximo);
		
		//correção: atualiza o anterior do próximo
		if (proximo != null) {
			proximo.setAnterior(anterior);
		} else {
			
			//correção: removendo o último elemento
			this.fim = anterior;
		}
		
		item.setProximo(null);
		
		//correção: adicionado o anterior
